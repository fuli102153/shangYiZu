...
export function bMapToQQMap(lng, lat) {
    ...
    var x = parseFloat(lng) - 0.0066;
    var y = parseFloat(lat) - 0.0053;
...
export function qqMapToBMap(lng, lat) {
    ...
    var z = Math.sqrt(x * x + y * y) + 0.00002 * Math.sin(y * x_pi);
    var theta = Math.atan2(y, x) + 0.000003 * Math.cos(x * x_pi);
    var lng = (z * Math.cos(theta) + 0.0066).toFixed(5);
    var lat = (z * Math.sin(theta) + 0.0053).toFixed(5);
    return [lng, lat];
...
//40、获取全部业态数据
export function getPropertyFormAllData(data){ return get(data,'open/propertyForm/allData')}

//41、获取某业态所有子级集合数据
export function getPropertyFormSubLevelData(data){ return get(data,'open/propertyForm/subLevelData')}

//42、精选品牌
export function getBrandSpecialList(data){ return post(data,'open/brand/specialList')}
